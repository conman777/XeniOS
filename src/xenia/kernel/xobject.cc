...
auto WaitWithTitleStopPollIOS(KernelState* kernel_state,
  ...
  while (true) {
    ...
    auto result =
        wait_call(TitleStopWaitSliceIOS(infinite_timeout, deadline));
  ...
  if (wait_type) {
...
#if XE_PLATFORM_IOS
        WaitWithTitleStopPollIOS(
            wait_kernel_state, timeout_ms,
            [&](std::chrono::milliseconds wait_timeout) {
              return xe::threading::WaitAny(wait_handles, count,
                                            alertable ? true : false,
                                            wait_timeout);
            });
#else
    ...
    auto result =
        ...
        WaitWithTitleStopPollIOS(
            wait_kernel_state, timeout_ms,
            [&](std::chrono::milliseconds wait_timeout) {
              return xe::threading::WaitAll(wait_handles, count,
                                            alertable ? true : false,
                                            wait_timeout);
            });
#else
