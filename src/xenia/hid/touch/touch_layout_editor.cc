...
bool IOSTouchLayoutHasActionBinding(const IOSTouchLayoutModel& layout,
  ...
  return std::any_of(
      layout.controls.begin(), layout.controls.end(),
...
std::string MakeUniqueIOSTouchActionButtonIdentifier(
  ...
  while (true) {
    std::string identifier =
        suffix == 1 ? "action_button" : ("action_button_" + std::to_string(suffix));
...
IOSTouchRect ClampIOSTouchEditorControlFrame(
    const IOSTouchRect& rect, IOSTouchControlType control_type) {
...
bool AddSuggestedActionButtonToIOSTouchLayout(
  ...
  control.normalized_frame =
      ClampIOSTouchEditorControlFrame(
...
bool DuplicateIOSTouchLayoutActionButton(
    IOSTouchLayoutModel* layout, std::size_t source_control_index,
