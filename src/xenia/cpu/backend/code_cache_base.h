 ...
 private:
  ...
  static bool AcquireIOSPersistentGeneratedCodeMapping(uint8_t*& execute_base,
    ...
    const kern_return_t remap_result = vm_remap(
        mach_task_self(), &remap_addr, kGeneratedCodeSize, 0,
        VM_FLAGS_ANYWHERE, mach_task_self(),
        reinterpret_cast<vm_address_t>(execute), FALSE, &cur_prot, &max_prot,
        VM_INHERIT_NONE);
    ...
    write_base = write;
    XELOGI(
        "iOS JIT persistent dual-mapping active: execute={:p} write={:p}",
