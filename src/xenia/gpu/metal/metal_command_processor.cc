...
bool MetalCommandProcessor::IssueDraw(xenos::PrimitiveType primitive_type,
  ...
  prepared_draw.texture_source_range_count =
...
bool MetalCommandProcessor::PrepareDrawConstants(
  ...
  auto try_reuse_or_upload =
      [&](ConstantBufferBinding& binding, size_t cbv_slot,
  ...
  if (!cbuffer_binding_system_.up_to_date) {
    if (!try_reuse_or_upload(
            cbuffer_binding_system_, kCbvSlotSystem, current_payload_system_,
  ...
  if (!cbuffer_binding_float_vertex_.up_to_date) {
    ...
    if (!try_reuse_or_upload(
            cbuffer_binding_float_vertex_, kCbvSlotFloat,
  ...
  if (!cbuffer_binding_float_pixel_.up_to_date) {
    ...
    const size_t float_size =
        ...
        std::max(float_map_pixel ? float_map_pixel->float_count : uint32_t(0),
                 uint32_t(1));
    if (!try_reuse_or_upload(
            cbuffer_binding_float_pixel_, kCbvSlotFloat,
  ...
  if (!cbuffer_binding_bool_loop_.up_to_date) {
    if (!try_reuse_or_upload(
            ...
            current_payload_bool_loop_, scratch_payload_bool_loop_,
...
void MetalCommandProcessor::MaybeDumpBackendTelemetry(const char* reason,
  ...
  const std::string texture_reload_reason_counts = MetalFormatNamedCounts(
      backend_telemetry_.texture_reload_reason_counts,
      MetalTextureReloadReasonName);
  const std::string texture_reload_reason_bytes = MetalFormatNamedCounts(
      backend_telemetry_.texture_reload_reason_bytes,
      MetalTextureReloadReasonName);
  const std::string texture_watch_invalidation_counts =
      MetalFormatNamedCounts(
          backend_telemetry_.texture_watch_invalidation_counts,
          MetalTextureWatchInvalidationReasonName);
  const std::string texture_watch_invalidation_bytes =
      MetalFormatNamedCounts(
          backend_telemetry_.texture_watch_invalidation_bytes,
          MetalTextureWatchInvalidationReasonName);
  const std::string texture_resolve_reload_counts = MetalFormatNamedCounts(
      backend_telemetry_.texture_resolve_reload_counts,
      MetalTextureResolveReloadReasonName);
  const std::string texture_resolve_reload_bytes = MetalFormatNamedCounts(
      backend_telemetry_.texture_resolve_reload_bytes,
      MetalTextureResolveReloadReasonName);
