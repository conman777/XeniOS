...
DEFINE_bool(halo_android_diag_direct_fb_fill_linear, false,
            ...
            "Android");
DEFINE_bool(halo_android_diag_direct_fb_fill_tiled, false,
            "Fill the Android Halo swap texture guest range as tiled 32bpp "
            "before texture-cache upload.",
            "Android");
DEFINE_bool(halo_android_diag_linear_to_tiled_frontbuffer, false,
            "Rewrite the Android Halo swap texture guest range from linear "
            "32bpp to tiled 32bpp before texture-cache upload.",
            "Android");
DEFINE_bool(halo_android_diag_force_null_textures, false,
            "Bind null images for guest draw texture descriptors on Android.",
            "Android");
DEFINE_bool(halo_android_diag_log_texture_bindings, false,
            "Log guest draw texture bindings on Android.",
            "Android");
DEFINE_uint32(halo_android_diag_swap_base_page_override, 0,
              "Override the Android Halo swap texture base 4 KB page for "
              "frontbuffer address diagnostics. 0 disables.",
              "Android");
...
#if XE_PLATFORM_ANDROID
bool AndroidHaloIsDxtEndianTestFormat(xenos::TextureFormat format) {
  switch (format) {
...
VkImageView VulkanTextureCache::GetActiveBindingOrNullImageView(
    ...
    bool is_signed) {
#if XE_PLATFORM_ANDROID
  static uint32_t android_texture_binding_log_count = 0;
#endif
  ...
  if (binding && AreDimensionsCompatible(dimension, binding->key.dimension)) {
    ...
    if (GetAndroidHaloExperiment().log_texture_bindings &&
      ...
      XELOGI(
          ...
          "signed={} base=0x{:08X} mip=0x{:08X} size={}x{}x{} pitch={} "
          "tiled={} format={} endian={} packed={} swizzle=0x{:03X}",
          android_texture_binding_log_count, fetch_constant_index,
          uint32_t(dimension), uint32_t(key.dimension), uint32_t(is_signed),
          uint32_t(key.base_page << 12), uint32_t(key.mip_page << 12),
          key.GetWidth(), key.GetHeight(), key.GetDepthOrArraySize(),
          uint32_t(key.pitch), uint32_t(key.tiled), uint32_t(key.format),
          uint32_t(key.endianness), uint32_t(key.packed_mips),
          uint32_t(binding->host_swizzle));
      ++android_texture_binding_log_count;
    }
    if (cvars::halo_android_diag_force_null_textures) {
      binding = nullptr;
    }
#endif
  }
...
bool VulkanTextureCache::ApplyAndroidDirectFrontbufferFill(
    uint32_t request_index, const TextureKey& key,
    const VulkanTexture& texture) {
#if XE_PLATFORM_ANDROID
  const bool fill_tiled = cvars::halo_android_diag_direct_fb_fill_tiled;
  const bool fill_linear =
      cvars::halo_android_diag_direct_fb_fill_linear && !fill_tiled;
  // The linear->tiled retiling assumed the compat hacks wrote LINEAR data
  // into the frontbuffer; the resolve path writes TILED data, so this remap
  // SCRAMBLES every presented frame (and burns ~20% of total CPU re-tiling
  // 830k pixels per swap - top hotspot in profiling 2026-06-10). Gated off by
  // default via the experiment file.
  const bool compat_linear_to_tiled =
      GetAndroidHaloExperiment().linear_to_tiled_frontbuffer &&
      cvars::halo_android_compat_linear_to_tiled_frontbuffer && !fill_tiled &&
      !fill_linear && key.GetWidth() == 1152 && key.GetHeight() == 720 &&
      key.format == xenos::TextureFormat::k_8_8_8_8 && key.tiled;
  const bool linear_to_tiled =
      (cvars::halo_android_diag_linear_to_tiled_frontbuffer ||
       compat_linear_to_tiled) &&
      !fill_tiled && !fill_linear;
  if (!fill_tiled && !fill_linear && !linear_to_tiled) {
    return false;
  }
  ...
  const uint32_t pitch_blocks = layout.base.row_pitch_bytes / bytes_per_block;
  if (!pitch_blocks || pitch_blocks < width) {
    return false;
  }
  ...
  Memory& memory = vulkan_shared_memory.guest_memory_for_diagnostics();
  uint8_t* base =
      reinterpret_cast<uint8_t*>(memory.TranslatePhysical(base_address));
  if (!base) {
    XELOGW("Android direct FB fill {} has null guest pointer at 0x{:08X}",
           request_index, base_address);
    return false;
  }

  const uint32_t bytes_per_block_log2 = xe::log2_floor(bytes_per_block);
  if (linear_to_tiled) {
    if (!key.tiled) {
      return false;
    }
    ...
    const size_t visible_bytes =
        size_t(width) * size_t(height) * size_t(bytes_per_block);
    std::vector<uint8_t> linear_pixels(visible_bytes);
    for (uint32_t y = 0; y < height; ++y) {
      for (uint32_t x = 0; x < width; ++x) {
        const uint32_t source_offset =
            (y * pitch_blocks + x) * bytes_per_block;
        const size_t linear_offset =
            (size_t(y) * size_t(width) + size_t(x)) * bytes_per_block;
        if (source_offset + bytes_per_block > base_size) {
          continue;
        }
        for (uint32_t i = 0; i < bytes_per_block; ++i) {
          linear_pixels[linear_offset + i] = base[source_offset + i];
        }
      }
    }

    shared_memory().MemoryInvalidationCallback(base_address, base_size, true);

    uint32_t pixels_written = 0;
    for (uint32_t y = 0; y < height; ++y) {
      for (uint32_t x = 0; x < width; ++x) {
        const uint32_t tiled_offset = uint32_t(texture_util::GetTiledOffset2D(
            int32_t(x), int32_t(y), pitch_blocks, bytes_per_block_log2));
        const size_t linear_offset =
            (size_t(y) * size_t(width) + size_t(x)) * bytes_per_block;
        if (tiled_offset + bytes_per_block > base_size) {
          continue;
        }
        for (uint32_t i = 0; i < bytes_per_block; ++i) {
          base[tiled_offset + i] = linear_pixels[linear_offset + i];
        }
        ++pixels_written;
      }
    }

    if (request_index <= 16 || request_index == 30 || request_index == 90 ||
        request_index == 180 || request_index == 600 ||
        (request_index % 1800) == 0) {
      XELOGI(
          "Android linear->tiled FB transform {}: addr=0x{:08X} key={}x{} "
          "pitch_blocks={} row_pitch_bytes={} base_size=0x{:X} "
          "pixels_written={} visible_bytes=0x{:X}",
          request_index, base_address, width, height, pitch_blocks,
          layout.base.row_pitch_bytes, base_size, pixels_written,
  ...
  for (uint32_t offset = 0; offset + 3 < base_size; offset += 4) {
    base[offset + 0] = 0x20;
    base[offset + 1] = 0x00;
    base[offset + 2] = 0x70;
    base[offset + 3] = 0xFF;
  ...
  for (uint32_t y = 0; y < height; ++y) {
    for (uint32_t x = 0; x < width; ++x) {
      const uint32_t offset_bytes =
          fill_tiled ? uint32_t(texture_util::GetTiledOffset2D(
      ...
      uint8_t r = uint8_t((uint64_t(x) * 255) / std::max(width - 1, 1u));
      uint8_t g = uint8_t((uint64_t(y) * 255) / std::max(height - 1, 1u));
      uint8_t b = ((x / 80) ^ (y / 16)) & 1 ? 255 : 32;

      const bool x_marker = x == 0 || x == 79 || x == 80 || x == 1119 ||
                            x == 1120 || x + 1 == width;
      const bool y_marker = y == 0 || y == 15 || y == 16 || y + 1 == height;
      ...
      base[offset_bytes + 0] = r;
      base[offset_bytes + 1] = g;
      base[offset_bytes + 2] = b;
      base[offset_bytes + 3] = 0xFF;
      ++pixels_written;
    }
  }
  ...
  if (request_index <= 16 || request_index == 30 || request_index == 90 ||
      ...
      (request_index % 1800) == 0) {
    XELOGI(
        "Android direct FB fill {}: mode={} addr=0x{:08X} key={}x{} "
        "pitch_blocks={} row_pitch_bytes={} tiled_key={} base_size=0x{:X} "
        "pixels_written={} visible_bytes=0x{:X}",
        request_index, fill_tiled ? "tiled" : "linear", base_address, width,
        height, pitch_blocks, layout.base.row_pitch_bytes, uint32_t(key.tiled),
        base_size, pixels_written, width * height * bytes_per_block);
  }
  return true;
#else
  (void)request_index;
...
VkImageView VulkanTextureCache::RequestSwapTexture(
    uint32_t frontbuffer_ptr, uint32_t& width_scaled_out,
...
#endif
#if XE_PLATFORM_ANDROID
  static uint32_t android_swap_texture_request_index = 0;
  uint32_t android_request_index = ++android_swap_texture_request_index;
  bool android_log_swap_texture =
      android_request_index <= 16 || android_request_index == 30 ||
      android_request_index == 90 || android_request_index == 180 ||
      android_request_index == 600 || android_request_index == 1200 ||
      (android_request_index % 1800) == 0;
#endif
  TextureKey key;
  BindingInfoFromFetchConstant(fetch, key, nullptr);
#if XE_PLATFORM_ANDROID
  const uint32_t android_pm4_frontbuffer_page = frontbuffer_ptr >> 12;
  const bool android_pm4_frontbuffer_usable =
      frontbuffer_ptr && !(frontbuffer_ptr & 0xFFF) &&
      android_pm4_frontbuffer_page != 0;
  if (key.is_valid && android_pm4_frontbuffer_usable &&
      key.dimension == xenos::DataDimension::k2DOrStacked &&
      key.GetWidth() == 1152 && key.GetHeight() == 720 &&
      key.format == xenos::TextureFormat::k_8_8_8_8 && key.tiled &&
      key.base_page != android_pm4_frontbuffer_page) {
    if (android_log_swap_texture) {
      XELOGI(
          "Android swap texture {} using PM4 frontbuffer page 0x{:X} over "
          "fetch page 0x{:X} for Halo-sized swap",
          android_request_index, android_pm4_frontbuffer_page,
          uint32_t(key.base_page));
    }
    key.base_page = android_pm4_frontbuffer_page;
  }

  // The experiment-file override is the reliable path; the cvar is kept as a
  // secondary source for old profiles.
  uint32_t android_swap_base_page_override =
      GetAndroidHaloExperiment().swap_base_page_override;
  if (!android_swap_base_page_override) {
    android_swap_base_page_override =
        cvars::halo_android_diag_swap_base_page_override;
  }
  if (android_swap_base_page_override && key.is_valid &&
      key.dimension == xenos::DataDimension::k2DOrStacked &&
      key.GetWidth() == 1152 && key.GetHeight() == 720 &&
      key.format == xenos::TextureFormat::k_8_8_8_8) {
    if (android_log_swap_texture) {
      XELOGI(
          "Android swap texture {} overriding base_page 0x{:X} -> 0x{:X}",
          android_request_index, uint32_t(key.base_page),
          android_swap_base_page_override);
    }
    key.base_page = android_swap_base_page_override;
  }
#endif
  if (!key.is_valid || key.base_page == 0 ||
    ...
    if (android_log_swap_texture) {
      XELOGI(
          "Android swap texture {} rejected: valid={} base_page=0x{:X} "
          "dimension={} pm4_fb=0x{:08X} fetch_dwords={:08X} {:08X} "
          "{:08X} {:08X} {:08X} {:08X}",
          android_request_index, uint32_t(key.is_valid),
          uint32_t(key.base_page),
          uint32_t(key.dimension), frontbuffer_ptr, fetch.dword_0,
  ...
  VkImageView texture_view = texture->GetView(
      false,
      GuestToHostSwizzle(android_swap_fetch_swizzle,
                         GetHostFormatSwizzle(key)),
      false);
#else
  VkImageView texture_view = texture->GetView(
...
#if XE_PLATFORM_ANDROID
  DumpAndroidCpuSwapTexture(android_request_index, key, *texture,
                            fetch.swizzle);
#endif
  texture->MarkAsUsed();
  VulkanTexture::Usage old_usage =
  ...
  if (android_log_swap_texture) {
    ...
    XELOGI(
        "Android swap texture {}: key={}x{} scaled={} pitch={} tiled={} "
        "packed_mips={} base_page=0x{:X} mip_page=0x{:X} format={} "
        "endian={} host_unsigned={} swizzle=0x{:03X} pm4_fb=0x{:08X} "
        "fetch_base=0x{:X} fetch_pitch={} fetch_tiled={} fetch_format={} "
        "fetch_endian={} fetch_size={}x{} fetch_dwords={:08X} {:08X} "
        "{:08X} {:08X} {:08X} {:08X}",
        android_request_index, key.GetWidth(), key.GetHeight(),
        uint32_t(key.scaled_resolve), uint32_t(key.pitch),
        uint32_t(key.tiled), uint32_t(key.packed_mips),
        uint32_t(key.base_page), uint32_t(key.mip_page),
        uint32_t(key.format), uint32_t(key.endianness),
        uint32_t(host_format_pair.format_unsigned.format),
        uint32_t(fetch.swizzle), frontbuffer_ptr, uint32_t(fetch.base_address),
        uint32_t(fetch.pitch), uint32_t(fetch.tiled),
        uint32_t(fetch.format), uint32_t(fetch.endianness),
        uint32_t(fetch.size_2d.width) + 1,
        uint32_t(fetch.size_2d.height) + 1, fetch.dword_0, fetch.dword_1,
...
bool VulkanTextureCache::DumpAndroidCpuSwapTexture(
    uint32_t request_index, const TextureKey& key,
    const VulkanTexture& texture, uint32_t fetch_swizzle) {
#if XE_PLATFORM_ANDROID
  // Fire at the early checkpoints, then every 300 swaps forever so dumps can
  // be captured at any later game state (menus, difficulty screen, gameplay).
  if (request_index != 120 && request_index != 240 && request_index != 360 &&
      request_index != 480 && (request_index % 300) != 0) {
    return false;
  }
  if (key.format != xenos::TextureFormat::k_8_8_8_8 ||
      key.dimension != xenos::DataDimension::k2DOrStacked) {
    XELOGI(
        "Android CPU swap texture {} skipped: unsupported format/dimension "
        "format={} dimension={}",
        request_index, uint32_t(key.format), uint32_t(key.dimension));
    return false;
  }
  ...
  const uint32_t bytes_per_block_log2 = xe::log2_floor(bytes_per_block);
  const uint32_t base_size = texture.GetGuestBaseSize();
  const uint32_t sampled_swizzle =
      GuestToHostSwizzle(fetch_swizzle, GetHostFormatSwizzle(key));

  VulkanSharedMemory& vulkan_shared_memory =
      static_cast<VulkanSharedMemory&>(shared_memory());
  Memory& memory = vulkan_shared_memory.guest_memory_for_diagnostics();
  const uint8_t* base = reinterpret_cast<const uint8_t*>(
      memory.TranslatePhysical(key.base_page << 12));
  if (!base) {
    XELOGW("Android CPU swap texture {} has null guest pointer",
           request_index);
    return false;
  }

  auto component_from_swizzle = [](const std::array<uint8_t, 4>& pixel,
                                   uint32_t swizzle,
                                   uint32_t output_component) -> uint8_t {
    uint32_t source_component = (swizzle >> (3 * output_component)) & 0b111;
    if (source_component < 4) {
      return pixel[source_component];
    }
    return source_component == xenos::XE_GPU_TEXTURE_SWIZZLE_1 ? 255 : 0;
  };
  ...
  struct VariantStats {
    ...
    uint64_t sum[3] = {};
    uint32_t nonblack = 0;
    // FNV-1a64 over the output RGB stream - byte-identical content across
    // frames/runs is visible from logcat alone, no file pull needed.
  ...
  const Variant variants[] = {
      {"raw_rgb", "", 0, false, false},
      {"raw_bgr", "_raw_bgr", 0, false, true},
      {"sampled_swizzle", "_sampled_swizzle", sampled_swizzle, true, false},
  };

  const uint64_t pixel_count = uint64_t(width) * height;
  std::string filename_base =
      "cpu_swap_texture_frame_" + std::to_string(request_index) + "_" +
      std::to_string(width) + "x" + std::to_string(height) + "_format_" +
      std::to_string(uint32_t(key.format));
  ...
  for (const char* diagnostic_root : kDiagnosticRoots) {
    ...
    std::filesystem::create_directories(diagnostic_root, ignored_error);
    if (ignored_error) {
      continue;
    }
    ...
    if (wrote_any || !dump_files) {
      ...
      for (uint32_t address_index = 0;
        ...
        const uint32_t guest_address =
            experiment.dump_addresses[address_index];
        const uint8_t* guest_base = reinterpret_cast<const uint8_t*>(
            memory.TranslatePhysical(guest_address));
        if (!guest_base) {
          continue;
        }
        ...
        for (uint32_t y = 0; y < kGuestDumpHeight; ++y) {
          for (uint32_t x = 0; x < kGuestDumpWidth; ++x) {
            ...
            for (uint32_t c = 0; c < 3; ++c) {
              ...
              nonblack |= guest_rgb[c] != 0;
              guest_stats.hash =
                  (guest_stats.hash ^ guest_rgb[c]) * 1099511628211ull;
            }
            if (nonblack) {
        ...
        const uint64_t guest_pixel_count =
            uint64_t(kGuestDumpWidth) * kGuestDumpHeight;
        const double guest_divisor = double(guest_pixel_count);
        char guest_raw_filename[64];
        std::snprintf(guest_raw_filename, sizeof(guest_raw_filename),
                      "raw_frame_%u_0x%08X.bin", request_index,
                      guest_address);
  ...
  XELOGW("Android CPU swap texture {} failed to open diagnostic output",
         request_index);
...
bool VulkanTextureCache::LoadTextureDataFromResidentMemoryImpl(Texture& texture,
  ...
  xenos::Endian texture_load_endianness = texture_key.endianness;
#if XE_PLATFORM_ANDROID
  const uint32_t android_tex_endian_mode =
      GetAndroidHaloExperiment().tex_endian_mode;
  if (android_tex_endian_mode != 0 &&
      AndroidHaloIsDxtEndianTestFormat(texture_key.format)) {
    const xenos::Endian original_endianness = texture_load_endianness;
    texture_load_endianness = android_tex_endian_mode == 1
                                  ? xenos::Endian::k8in16
                                  : xenos::Endian::kNone;
    static uint32_t android_tex_endian_override_log_count = 0;
    if (android_tex_endian_override_log_count < 96) {
      XELOGI(
          "Android tex_endian override {}: mode={} format={} size={}x{}x{} "
          "base_page=0x{:X} mip_page=0x{:X} tiled={} endian {} -> {} "
          "load_shader={} host_format={} block_compressed={}",
          android_tex_endian_override_log_count, android_tex_endian_mode,
          uint32_t(texture_key.format), width, height, depth_or_array_size,
          uint32_t(texture_key.base_page), uint32_t(texture_key.mip_page),
          uint32_t(texture_key.tiled), uint32_t(original_endianness),
          uint32_t(texture_load_endianness), uint32_t(load_shader),
          uint32_t(host_format.format), uint32_t(host_format.block_compressed));
