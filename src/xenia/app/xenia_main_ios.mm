...
std::filesystem::path IOSPatchStoragePath(const std::filesystem::path& storage_root,
  ...
  return storage_root / "patches" / patch_file.filename;
}
...
std::vector<patcher::BundledPatchFile> EnumerateIOSPatchFilesForTitle(
  ...
  if (summary_out) {
    summary_out->directory_path = xe::path_to_utf8((storage_root / "patches").lexically_normal());
  ...
  const std::filesystem::path patches_dir = storage_root / "patches";
